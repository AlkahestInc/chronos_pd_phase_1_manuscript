...
here::i_am("src/20_analyses/target_validation/gnpc/data_preparation/cohort_L_matching_sample_prep.R")
...
metadata <- glue::glue(
  "{input_dir}/data_prep/metadata/clinical_metadata_tbl_v6_contributor_L.rds",
) |>
  readr::read_rds() |>
  dplyr::filter(
    !is.na(pd)
...
protein_intensity_log2 <- glue::glue(
  "{input_dir}/data_prep/proteomics/proteomic_tbl_v6_contributor_L.rds",
) |>
  readr::read_rds() |>
  dplyr::semi_join(
    metadata,
...
metadata_output_stem <- glue::glue("{get_config()$checkpoint_dir}/data_prep/gnpc_cohort_cleanup_matched_data/cohort_L_outliers_rm_2to1_matched_grp_filtered_metadata")
protein_intensity_log2_output_stem <- glue::glue("{get_config()$checkpoint_dir}/data_prep/gnpc_cohort_cleanup_matched_data/cohort_L_outliers_rm_2to1_matched_grp_filtered_protein_intensity_log2")
