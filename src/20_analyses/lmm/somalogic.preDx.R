...
here::i_am("src/20_analyses/lmm/somalogic.preDx.R")
...
output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/somalogic.preDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/somalogic.preDx.screening.rds")
...
# preDx model ####
ID_pre <- somalogic_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset < 0)

somalogic_tibble_pre <- list(
