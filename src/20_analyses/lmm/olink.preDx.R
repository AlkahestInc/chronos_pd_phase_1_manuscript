...
here::i_am("src/20_analyses/lmm/olink.preDx.R")
...
output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/olink.preDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/olink.preDx.screening.rds")
...
# preDx model ####
ID_pre <- olink_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset < 0)

olink_tibble_pre <- list(
