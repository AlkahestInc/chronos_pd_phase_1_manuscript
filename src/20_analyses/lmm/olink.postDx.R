...
here::i_am("src/20_analyses/lmm/olink.postDx.R")

# screen model ####
input_olink_dataset_file <- glue::glue("{get_config()$input_dir}/name=Chronos_phase1_proteomics_olink_analysis_ready_three_tibble/version=29465/dataset.rds")

output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/olink.postDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/olink.postDx.screening.rds")
...
olink_tibble <- readRDS(input_olink_dataset_file)

# postDx model ####
ID_post <- olink_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset >= 0)

olink_tibble_post <- list(
  prot_data = olink_tibble$prot_data |>
    dplyr::filter(ID %in% ID_post$ID) |>
    tibble::as_tibble(),
  meta_data = ID_post |>
    tibble::as_tibble(),
  prot_annot = olink_tibble$prot_annot |>
    tibble::as_tibble()
)
