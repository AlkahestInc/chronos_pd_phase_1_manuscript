...
here::i_am("src/20_analyses/lmm/alamar.postDx.R")
...
output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/alamar.postDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/alamar.postDx.screening.rds")
...
# postDx model ####
ID_post <- alamar_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset >= 0)

alamar_tibble_post <- list(
