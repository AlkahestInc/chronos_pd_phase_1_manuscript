...
here::i_am("src/20_analyses/lmm/biognosys.preDx.R")
...
output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/biognosys.preDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/biognosys.preDx.screening.rds")
...
# preDx model ####
ID_pre <- biognosys_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset < 0)

biognosys_tibble_pre <- list(
...
grouppd_table |>
  dplyr::filter(p.value < 0.05) # 0 adjp; 346 pvalue
