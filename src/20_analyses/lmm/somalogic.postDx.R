...
here::i_am("src/20_analyses/lmm/somalogic.postDx.R")

# screen model ####
input_somalogic_dataset_file <- glue::glue("{get_config()$input_dir}/name=Chronos_phase1_proteomics_somalogic_analysis_ready_three_tibble/version=29650/dataset.rds")

output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/somalogic.postDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/somalogic.postDx.screening.rds")
...
somalogic_tibble <- readRDS(input_somalogic_dataset_file)

# postDx model ####
ID_post <- somalogic_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset >= 0)

somalogic_tibble_post <- list(
  prot_data = somalogic_tibble$prot_data |>
    dplyr::filter(ID %in% ID_post$ID) |>
    tibble::as_tibble(),
  meta_data = ID_post |>
    tibble::as_tibble(),
  prot_annot = somalogic_tibble$prot_annot |>
    tibble::as_tibble()
)
