...
here::i_am("src/20_analyses/lmm/biognosys.postDx.R")

# screen model ####
input_biognosys_dataset_file <- glue::glue("{get_config()$input_dir}/name=Chronos_phase1_proteomics_biognosys_analysis_ready_three_tibble/version=29639/dataset.rds")

output_intermediate_result_file <- glue::glue("{get_config()$output_dir}/lmm/intermediate/biognosys.postDx.screening.rds")
output_result_file <- glue::glue("{get_config()$output_dir}/lmm/biognosys.postDx.screening.rds")
...
biognosys_tibble <- readRDS(input_biognosys_dataset_file)

# postDx model ####
ID_post <- biognosys_tibble$meta_data |>
  dplyr::filter(years_since_pd_onset >= 0)

biognosys_tibble_post <- list(
  prot_data = biognosys_tibble$prot_data |>
    dplyr::filter(ID %in% ID_post$ID) |>
    tibble::as_tibble(),
  meta_data = ID_post |>
    tibble::as_tibble(),
  prot_annot = biognosys_tibble$prot_annot |>
    tibble::as_tibble()
)
...
random_effects_list <- c(
  "1 | patient_id + 1 | PREP_PLATE_NUMBER",
  "1 | patient_id + 1 | center + 1 | PREP_PLATE_NUMBER",
  "1 + years_since_pd_onset | patient_id + 1 | PREP_PLATE_NUMBER",
  "1 | patient_id + (0 + years_since_pd_onset | patient_id) + 1 | PREP_PLATE_NUMBER"
)

res <- pbmcapply::pbmclapply(biognosys_tibble_post$prot_annot |>
                               dplyr::pull(ID), # Iterate through protein IDs
                             compare_multiple_models_low_mem_wrapper, ### <- The function being ran
                             three_tibble_list = biognosys_tibble_post,
...
grouppd_table |>
  dplyr::filter(p.value < 0.05) # 0 adjp; 543 pvalue
