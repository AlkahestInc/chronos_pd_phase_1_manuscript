...
here::i_am("src/20_analyses/joint_modeling/comorbidity/model_processing/Alamar_PD_comorbidity.R")

# JM functions are loaded automatically from packages/manuscript via .Rprofile
# data loading ####
cli::cli_h1("Alamar PD Comorbidity Joint Modeling")
...
JM_lme_alamar <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JM_lme_alamar/version=none/dataset.parquet"))
JM_surv_alamar <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JM_surv_alamar/version=none/dataset.parquet"))

# Get all comorbidity codes (3-digit ICD10 codes) in this dataset
diagnosis_code_list <- sort(unique(JM_surv_alamar$diagnosis_code_3))
...
for (diag_index in seq_along(diagnosis_code_list)) {
  ...
  for (protein_index in seq_along(alamar_proteins)) {
    ...
    tryCatch({
      ...
      long_sub <- JM_lme_alamar  |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
      surv_sub <- JM_surv_alamar |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
