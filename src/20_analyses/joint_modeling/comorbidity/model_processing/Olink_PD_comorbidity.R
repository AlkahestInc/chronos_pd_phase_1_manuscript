...
here::i_am("src/20_analyses/joint_modeling/comorbidity/model_processing/Olink_PD_comorbidity.R")

# JM functions are loaded automatically from packages/manuscript via .Rprofile
# data loading ####
cli::cli_h1("Olink PD Comorbidity Joint Modeling")
...
JM_lme_olink <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JM_lme_olink/version=none/dataset.parquet"))
JM_surv_olink <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JM_surv_olink/version=none/dataset.parquet"))

# Get all comorbidity codes (3-digit ICD10 codes) in this dataset
diagnosis_code_list <- sort(unique(JM_surv_olink$diagnosis_code_3))
...
for (diag_index in seq_along(diagnosis_code_list)) {
  ...
  for (protein_index in seq_along(olink_proteins)) {
    ...
    tryCatch({
      ...
      long_sub <- JM_lme_olink  |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
      surv_sub <- JM_surv_olink |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
