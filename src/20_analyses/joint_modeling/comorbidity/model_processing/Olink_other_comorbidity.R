...
here::i_am("src/20_analyses/joint_modeling/comorbidity/model_processing/Olink_other_comorbidity.R")

# JM functions are loaded automatically from packages/manuscript via .Rprofile
# data loading ####
cli::cli_h1("Olink Other Comorbidity Joint Modeling")
...
JMcondition_lme_olink <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JMcondition_lme_olink/version=none/dataset.parquet"))
JMcondition_surv_olink <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JMcondition_surv_olink/version=none/dataset.parquet"))

# Get all comorbidity codes (3-digit ICD10 codes) in this dataset
diagnosis_code_list <- sort(unique(JMcondition_surv_olink$diagnosis_code_3))
n_diag <- length(diagnosis_code_list)

# Olink proteins only
olink_idx      <- which(protein_list$platform == "Olink_HT")
olink_proteins <- protein_list$probe_id[olink_idx]
n_prot        <- length(olink_proteins)

cli::cli_alert_info(glue::glue(
  "Found {n_diag} diagnosis codes and {n_prot} Olink proteins"
))
cli::cli_alert_info(glue::glue(
...
for (diag_index in seq_along(diagnosis_code_list)) {
  ...
  for (protein_index in seq_along(olink_proteins)) {
    protein_subset <- olink_proteins[protein_index]

    # Define output filepaths
    output_file_full <- glue::glue("{output_base_full}/olink_JM.{diag_code}.{protein_index}.{protein_subset}.RDS")
    output_file_summary <- glue::glue("{output_base_summary}/olink_JM.{diag_code}.{protein_index}.{protein_subset}.RDS")
    ...
    tryCatch({
      ...
      long_sub <- JMcondition_lme_olink  |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
      surv_sub <- JMcondition_surv_olink |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
