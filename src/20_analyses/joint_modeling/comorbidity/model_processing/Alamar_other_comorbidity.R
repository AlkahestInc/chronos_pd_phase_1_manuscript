...
here::i_am("src/20_analyses/joint_modeling/comorbidity/model_processing/Alamar_other_comorbidity.R")

# JM functions are loaded automatically from packages/manuscript via .Rprofile
# data loading ####
cli::cli_h1("Alamar Other Comorbidity Joint Modeling")
...
JMcondition_lme_alamar <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JMcondition_lme_alamar/version=none/dataset.parquet"))
JMcondition_surv_alamar <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JMcondition_surv_alamar/version=none/dataset.parquet"))

# Get all comorbidity codes (3-digit ICD10 codes) in this dataset
diagnosis_code_list <- sort(unique(JMcondition_surv_alamar$diagnosis_code_3))
n_diag <- length(diagnosis_code_list)

# Alamar proteins only
alamar_idx      <- which(protein_list$platform == "Alamarbio_CNS")
alamar_proteins <- protein_list$probe_id[alamar_idx]
n_prot        <- length(alamar_proteins)

cli::cli_alert_info(glue::glue(
  "Found {n_diag} diagnosis codes and {n_prot} Alamar proteins"
))
cli::cli_alert_info(glue::glue(
...
output_base_full <- glue::glue("{get_config()$checkpoint_dir}/joint_modeling/comorbidity/other_comorbidity/full_results")
output_base_summary <- glue::glue("{get_config()$checkpoint_dir}/joint_modeling/comorbidity/other_comorbidity/summary_only")
...
for (diag_index in seq_along(diagnosis_code_list)) {
  ...
  for (protein_index in seq_along(alamar_proteins)) {
    protein_subset <- alamar_proteins[protein_index]

    # Define output filepaths
    output_file_full <- glue::glue("{output_base_full}/alamar_JM.{diag_code}.{protein_index}.{protein_subset}.RDS")
    output_file_summary <- glue::glue("{output_base_summary}/alamar_JM.{diag_code}.{protein_index}.{protein_subset}.RDS")
    ...
    tryCatch({
      ...
      long_sub <- JMcondition_lme_alamar  |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
      surv_sub <- JMcondition_surv_alamar |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
