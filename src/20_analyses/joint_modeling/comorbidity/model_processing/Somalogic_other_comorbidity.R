...
here::i_am("src/20_analyses/joint_modeling/comorbidity/model_processing/Somalogic_other_comorbidity.R")

# JM functions are loaded automatically from packages/manuscript via .Rprofile
# data loading ####
cli::cli_h1("Somalogic Other Comorbidity Joint Modeling")
...
JMcondition_lme_somalogic <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JMcondition_lme_somalogic/version=none/dataset.parquet"))
JMcondition_surv_somalogic <- arrow::read_parquet(glue::glue("{get_config()$checkpoint_dir}/name=JMcondition_surv_somalogic/version=none/dataset.parquet"))

# Get all comorbidity codes (3-digit ICD10 codes) in this dataset
diagnosis_code_list <- sort(unique(JMcondition_surv_somalogic$diagnosis_code_3))
...
for (diag_index in seq_along(diagnosis_code_list)) {
  ...
  for (protein_index in seq_along(soma_proteins)) {
    ...
    tryCatch({
      ...
      long_sub <- JMcondition_lme_somalogic  |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
      surv_sub <- JMcondition_surv_somalogic |> dplyr::filter(diagnosis_code_3 == diag_code, age_entry < age_event)
